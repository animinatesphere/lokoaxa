...
const ProductCard = ({ product, onRemove }) => (
  <div className="flex flex-col gap-3 group">
    <Link to={`/product/${product.id}`} className="relative rounded-2xl overflow-hidden aspect-[4/5] bg-gray-100 block">
      <img 
        src={product.image} 
        alt={product.name} 
      ...
      />
      {product.badge && (
        <div className={`absolute top-3 left-3 px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${product.badgeColor}`}>
          {product.badge}
        </div>
      )}
      <button 
      ...
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6">
          ...
          <div className="w-24 h-24 rounded-full bg-purple-100 flex items-center justify-center">
            <Heart size={56} className="text-[#B000FF]" />
          </div>

          <div className="text-center max-w-md">
            <h2 className="text-2xl font-bold text-gray-900 mb-3">Your Wishlist is Empty</h2>
            <p className="text-gray-500 text-sm mb-8">
              Bookmark items on the Discover runway by toggling the heart icons to save your preferred styles here.
            </p>
          </div>

          <Link
            to="/discover"
            className="py-3 px-8 bg-[#B000FF] text-white rounded-xl font-medium hover:bg-purple-700 transition-colors"
          >
            Examine Catalog
          </Link>
        </div>
      ...
      <div className="flex flex-col gap-8 pb-12">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500 text-sm">You have {wishlistItems.length} items saved in your wishlist</p>
          </div>
          <button
            onClick={handleAddAllToCart}
            className="py-2 px-6 bg-[#B000FF] text-white rounded-lg font-medium hover:bg-purple-700 transition-colors text-sm"
          >
            Add all to cart
          </button>
        </div>
