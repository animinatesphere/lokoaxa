      ...
      <div className="flex flex-col gap-8 pb-12">
        ...
        <div className="w-full h-[320px] rounded-3xl overflow-hidden relative flex bg-gradient-to-r from-[#D4BEE4] via-[#E9D9F2] to-white">
          ...
          <div className="absolute inset-0 flex justify-between px-12 pointer-events-none opacity-90">
             <img src={heroBannerData.image1} alt="banner-bg-1" className="h-[120%] object-cover -mt-10 -ml-10 mix-blend-multiply opacity-80" />
             <img src={heroBannerData.image2} alt="banner-bg-2" className="h-[120%] object-cover -mt-10 mr-20" />
          </div>

          <div className="relative z-10 flex flex-col justify-center h-full px-12 max-w-lg">
            <span className="px-4 py-1.5 rounded-full border border-gray-800 text-gray-800 text-xs font-semibold w-max mb-4">
              {heroBannerData.badge}
            </span>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">
              {heroBannerData.title}
            </h1>
            <p className="text-gray-700 text-sm leading-relaxed max-w-sm whitespace-pre-line">
                ...
                return (
                  ...
                  >
                    <div className={`w-[60px] h-[60px] rounded-full flex items-center justify-center transition-all ${
                      isActive ? 'border-2 border-[#B000FF] p-1' : 'border-2 border-transparent p-1'
                    }`}>
                    ...
                    <span className={`text-sm ${isActive ? 'text-[#B000FF] font-medium' : 'text-gray-600'}`}>
                      {subCat.name}
                    </span>
                  </div>
                );
        ...
        <div className="flex flex-col gap-4 mt-4">
          <h2 className="text-lg font-bold text-gray-900">Best deal For You</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {mockProducts.map((product) => (
